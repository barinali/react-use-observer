...
const IntersectionObserverStory = () => {
  // `false` to see the problematic case
  // `true` to have it in the happy path
  ...
  return (
    ...
    >
      ...
      {visible && (
        ...
        >
          <div>{`${ratio}%`}</div>
          <span>{intersectionObserverEntry instanceof IntersectionObserverEntry ? 'has an observer entry' : 'doesn\'t have an observer entry'}</span>
          <span
            style={{
              position: 'absolute',
              bottom: 0,
...
storiesOf('useIntersectionObserver', module)
  .add('async-ref', () => <IntersectionObserverStory />)
